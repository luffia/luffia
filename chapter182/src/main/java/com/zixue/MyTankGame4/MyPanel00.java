...
//        }
//        将 hero 的子弹 集合shots 遍历取出绘制
        for (int i = 0; i < hero02.shots.size(); i++) {
            Shot shot = hero02.shots.get(i);
            if (shot !=null && shot.isLive==true){
                g.draw3DRect(shot.x,shot.y,3,3,false);
            }else {//如果shot 对象已经无效，那么就从shots集合中拿掉
                hero02.shots.remove(shot);
            }
        }
...
//                    c创建一个Bomb对象，加入到bombs集合中
    ...
    public void keyPressed(KeyEvent e) {
        if (e.getKeyCode() == KeyEvent.VK_W) {
            ...
            hero02.setDirect(0);
            //修改坦克坐标
            if (hero02.getY()>0) {//自己坦克的移动范围
                hero02.moveUp();
            }
        } else if (e.getKeyCode() == KeyEvent.VK_D) {//D向右
            hero02.setDirect(1);
            if (hero02.getX()+60<1000) {
                hero02.moveRight();
            }
        } else if (e.getKeyCode() == KeyEvent.VK_S) {
            hero02.setDirect(2);
            if (hero02.getY()+60<750) {
                hero02.moveDown();
            }
...
//            if(hero02.shot==null || !hero02.shot.isLive){ //
